...
# telemetry <- read.csv("https://azuremlsampleexperiments.blob.core.windows.net/datasets/PdM_telemetry.csv")
# 一度ダウンロードしたらローカルから読み込む
telemetry <- read.table("PdM_telemetry.csv", header = T, sep = ",")
head(telemetry, 10)

# format datetime field which comes in as.character
# telemetry$datetime <- as.POSIXct(telemetry$datetime, format="%m/%d/%Y %I:%M:%S %p", tz="UTC")
...
# errors <- read.csv("https://azuremlsampleexperiments.blob.core.windows.net/datasets/PdM_errors.csv")
# 一度ダウンロードしたらローカルから読み込む
errors <- read.table("PdM_errors.csv", header = T, sep = ",")
head(errors, 10)

# format datetime and errorID fields
# errors$datetime <- as.POSIXct(errors$datetime, format="%m/%d/%Y %I:%M:%S %p", tz="UTC")
...
#ggplot(errors, aes(x = errorID)) + geom_histogram(fill = "orange") + labs(title = "Errors by type", x = "error types")
ggplot(errors, aes(x = errorID)) + geom_histogram(fill = "orange", stat = "count") + labs(title = "Errors by type", x = "error types")

options(repr.plot.width = 6, repr.plot.height = 5)
#ggplot(errors %>% filter(machineID < 4), aes(x = errorID, fill = factor(machineID))) + geom_histogram(color = "black") + labs(title = "MachineID errors by type", x = "error types", fill = "MachineID") + facet_wrap(~machineID, ncol = 1)
ggplot(errors %>% filter(machineID < 4), aes(x = errorID, fill = factor(machineID))) + geom_histogram(color = "black", stat = "count") + labs(title = "MachineID errors by type", x = "error types", fill = "MachineID") + facet_wrap(~machineID, ncol = 1)
...
# maint <- read.csv("https://azuremlsampleexperiments.blob.core.windows.net/datasets/PdM_maint.csv")
# 一度ダウンロードしたらローカルから読み込む
maint <- read.table("PdM_maint.csv", header = T, sep = ",")
head(maint, 10)

# format datetime and comp fields
# maint$datetime <- as.POSIXct(maint$datetime, format = "%m/%d/%Y %I:%M:%S %p", tz = "UTC")
...
ggplot(maint, aes(x = comp)) +
    #geom_histogram(fill = "magenta") + labs(title = "Component replacements", x = "component types")
    geom_histogram(fill = "magenta", stat = "count") + labs(title = "Component replacements", x = "component types")

options(repr.plot.width = 6, repr.plot.height = 8)
ggplot(maint %>% filter(machineID < 4), 
    aes(x = comp, fill = factor(machineID))) +
    # geom_histogram(color = "black") + labs(title = "Component replacements", x = "component types", fill = "Machine ID") + facet_wrap(~machineID, ncol = 1)
    geom_histogram(color = "black", stat = "count") + labs(title = "Component replacements", x = "component types", fill = "Machine ID") + facet_wrap(~machineID, ncol = 1)

options(repr.plot.width = 7, repr.plot.height = 5)
ggplot(maint %>% filter(machineID == 4), 
    aes(y = comp, x = datetime)) +
    geom_point(color = "black", alpha = 0.5) + labs(title = "MachineID 4 component replacements", x = "Date")

# download machines dataset
# machines <- read.csv("https://azuremlsampleexperiments.blob.core.windows.net/datasets/PdM_machines.csv")
# 一度ダウンロードしたらローカルから読み込む
machines <- read.table("PdM_machines.csv", header = T, sep = ",")
head(machines, 10)
...
ggplot(machines, aes(x = age, fill = model)) +
    # geom_histogram(color = "black") + labs(title = "Machines", x = "age (years)") + facet_wrap(~model)
    geom_histogram(color = "black", stat = "count") + labs(title = "Machines", x = "age (years)") + facet_wrap(~model)

# download failures dataset
# failures <- read.csv("https://azuremlsampleexperiments.blob.core.windows.net/datasets/PdM_failures.csv")
# 一度ダウンロードしたらローカルから読み込む
failures <- read.table("PdM_failures.csv", header = T, sep = ",")
head(failures, 10)

# format datetime and failure fields
# failures$datetime <- as.POSIXct(failures$datetime, format="%m/%d/%Y %I:%M:%S %p", tz="UTC")
...
ggplot(failures, aes(x = failure)) +
    #geom_histogram(fill = "red") + labs(title = "Failure distribution", x = "component type")
    geom_histogram(fill = "red", stat = "count") + labs(title = "Failure distribution", x = "component type")


options(repr.plot.width = 6, repr.plot.height = 6)
ggplot(failures %>% filter(machineID < 4),
    aes(x = failure, fill = factor(machineID))) +
    #geom_histogram(color = "black") + labs(title = "Failure distribution", x = "component type", fill = "MachineID") + facet_wrap(~machineID, ncol = 1)
    geom_histogram(color = "black", stat = "count") + labs(title = "Failure distribution", x = "component type", fill = "MachineID") + facet_wrap(~machineID, ncol = 1)
